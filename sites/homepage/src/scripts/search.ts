...
async function search() {
    ...
    function findTitles() {
        ...
        links.forEach((link, i) => link.title.toLowerCase().indexOf(query) > -1 && titles.push([i, link.title.toLowerCase().indexOf(query)]))
    ...
    function findTags() {
        ...
        links.forEach((link, i) => link.tags.forEach((tag, j) => tag.indexOf(query) > -1 && tags.push([i, j])))
    ...
    function findDesc() {
        ...
        links.forEach((link, i) => (link.desc && link.desc.toLowerCase().indexOf(query) > -1) && descs.push([i, link.desc.toLowerCase().indexOf(query)]))
