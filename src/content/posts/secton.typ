...
#note(
  ...
  I'd like to give credit to my friends, #link("https://codycody31.dev")[InsidiousFiddler] and #link("https://xnoid.studio")[RollViral], for helping out with this project. InsidiousFiddler has his own write-up of this, which you can find #link("https://codycody31.dev/when-security-is-not-a-factor-secton/")[here]. You can read that one or this one, they have basically the same content.
]
...
The server wasn't checking where the request was coming from, and I could call the API on my own without any authentication or limits! Using this, over the span of around 20 mins, RollViral and I built (using #link("https://claude.ai")[Claude]) a small proof of concept of a chat UI using this API and deployed it. This allowed us to have unlimited access to Copilot without facing the annoyance of having to remove the ```html disabled``` attribute.

#figure(
  image("secton/copilot-poc.png"),
  caption: [Our proof of concept website using the Copilot API],
)
...
#figure(
  image("secton/platform-playground.png"),
  caption: [A successful HTTP request using the `playground` Bearer Token],
)
...
#figure(
  image("secton/speech-compute.png"),
  caption: [A failing HTTP request to `speech-compute.secton.org`],
)
