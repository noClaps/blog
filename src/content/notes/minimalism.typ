...
There's an idea in programming called the UNIX philosophy, which basically boils down to, "do one thing well". For example, there's a command line tool called ```sh cd```, which stands for "Change Directory". That's all it does, you can use it to change the directory you're in. Let's say you're in the `~/Downloads/` directory and you want to go to the `~/Documents/` directory. All you need to do is run:

```sh
cd ~/Documents/
```

However, you can't use ```sh cd``` to create new files or directories, or rename them or move them around. It does one thing, changing the current directory you're in, and it does it well.

There's something nice about how simple and intentional it is. There's no confusion about what's going to happen when I run ```sh cd``` because it can only do one thing. That's kind of how I try to make my own programs. I made a command line tool called #link("https://github.com/noClaps/random")[```sh random```], and all it can do is generate random numbers, and flip a coin. I made another tool called #link("https://github.com/noClaps/lsdeps")[```sh lsdeps```] that lists the number of dependencies a package on npm has.

```sh lsdeps``` is an interesting one, because my motivation for it was to show how bad the dependency problem is in JavaScript land. I go into more depth with my reasoning in the README file in the repository, so if you want to read that, #link("https://github.com/noClaps/lsdeps#motivation")[here you go]. Essentially, I was tired of having to install thousands of packages just to make something as simple as a static blog or gallery website, only to have stuff break when I updated it. I was tired of having occasional security warnings for packages I didn't even know were part of the repo. I was tired of frameworks trying to do everything at once.
